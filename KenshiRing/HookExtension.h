...
* //----------------------------------------//
*
* 1. создайте свою функцию с параметрами "inline void yourNameFunction(Param& p1, Param& p2, Param& p3, Param& p4, ... )" В Setup -> CustomEffects -> "Category" -> name.h
* name.h разделены по смыслу между собой в зависимости от логики воспроизведения
* 
...
*    пример расписан в Setup -> CustomEffects -> Attack -> YouTakingHit.h
* 
...
* 
* 4. создайте новый color с именем эффекта выбранным раньше, в начало добавьте символ #, получится так "#yourNameEffectFoo", так же добавтье в конец к нему 13 любых символа после, например "-050-100_____" 
*    для упрощения ориентировке между одинаковыми названиями, или копирования, или своих тегов,
* 
*    итоговое значение что то вроде "#yourNameEffectFoo-050-100 copy" (если скопируете шаблон #zTemplateForCopy-...-...) или "#yourNameEffectFoo12345678901234" или "#yourNameEffectFoo----====+++++" по вашему желанию 
*    последнии 13 символов не будут учитываться у названия вашего цвета
* 
* 5. так как игра будет сравнивать "#yourNameEffectFoo" добавленный программно, и имя color у "#yourNameEffectFoo-050-100_____" без последних 13 символов, 
...
*
* 6. измените ПЕРВОЕ значение у параметра color1 (0-100), это КРАСНЫЙ цвет, у нашего "#yourNameEffectFoo-050-100_____" - например на 50 (так как мы ранее обозначили его в "-050" для удобства)
...
* 
* 7. измените ВТОРОЕ значение у параметра color1 (0-255), это ЗЕЛЕНЫЙ цвет, у нашего "#yourNameEffectFoo-050-100_____" - например на 100 (так как мы ранее обозначили его в "-100" для удобства)
